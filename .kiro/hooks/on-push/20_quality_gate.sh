...
# Compliance: Enforces NFR-003 (Usability) from specs/requirements.md. (Required Coverage: 85%)
...
# Mock check for test coverage. In a real environment, this would parse a coverage report (e.g., from 'go test -coverprofile' or 'pytest --cov'). **(Enforced via MCP servers like `code-analysis` or CI pipeline).**
