# التوجيه الأمني والموثوقية (Security and Reliability Steering)
...
1.  **المرونة الهندسية:** يجب على الوكيل التأكد من أن الكود المولد يطبق أنماط المرونة المحددة في `structure.md` (مثل Circuit Breakers و Retries) لتقليل وقت التعافي من الفشل (Time to Restore).
2.  **التكرار (Redundancy):** يجب أن يتم تصميم البنية التحتية (في `iac/`) لضمان التكرار عبر مناطق التوفر (Availability Zones) لتقليل نقاط الفشل الفردية (Single Points of Failure).
3.  **المراقبة والتشخيص:** يجب أن يتضمن الكود المولد آليات تسجيل (Logging) وتتبع (Tracing) شاملة لتمكين التشخيص السريع للمشكلات، مما يدعم مقياس **Time to Restore** في DORA.
