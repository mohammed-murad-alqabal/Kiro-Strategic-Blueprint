...
## 3. Non-Functional Requirements (NFRs) and Enforcement Policies
*   **NFR-001 (Performance):** The registration API endpoint response time **shall** be less than 500ms for 99% of requests.
*   **NFR-002 (Security - Secrets):** All password hashes **shall** be stored using a strong, industry-standard algorithm (e.g., Argon2). **(Enforced by `snyk-vuln-scan` and `iac-scan` in MCP).**
*   **NFR-003 (Usability):** The entire onboarding flow **shall** be completable within 3 steps after initial form submission.

*   **NFR-004 (Supply Chain Integrity):** All published artifacts **shall** be signed using a private key. **(Enforced by `artifact-sign` in MCP).**\n*   **NFR-005 (Container Security):** All container images **shall** be scanned for critical vulnerabilities before deployment. **(Enforced by `container-scan` in MCP).**\n\n## 4. Architectural Considerations
